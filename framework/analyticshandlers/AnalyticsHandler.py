...
class AnalyticsHandler(BaseDBHandler):
                ...
                if config.DB_TYPE == 'postgres':
                    if pushSource == PushSource.API:
                        query = text('''
                            ...
                            WHERE source = %s AND active = TRUE AND status = %s AND superuser = 1
                        ''')
                        params = (source, StrategyStatus.ACTIVE.value)
                    else:
                        query = text('''
                            SELECT * FROM strategyconfig 
                            WHERE source = %s AND active = TRUE AND status = %s AND superuser = 0
                        ''')
                        params = (source, StrategyStatus.ACTIVE.value)
                    cursor.execute(query, params)
                else:
    ...
    def getStrategyExecutions(self, strategyId: int) -> List[Dict]:
        ...
        try:
            ...
            with self.conn_manager.transaction() as cursor:
                if config.DB_TYPE == 'postgres':
                    query = text('SELECT * FROM strategyexecution WHERE strategyid = %s')
                    params = (strategyId,)
                    cursor.execute(query, params)
                else:
    ...
    def getExecutionTrades(self, executionId: int) -> List[Dict]:
        ...
        try:
            ...
            with self.conn_manager.transaction() as cursor:
                if config.DB_TYPE == 'postgres':
                    query = text('SELECT * FROM tradelog WHERE executionid = %s ORDER BY createdat ASC')
                    params = (executionId,)
                    cursor.execute(query, params)
                else:
    ...
    def updateExecution(self, executionId: int, investedAmount: Decimal = None, 
        ...
        try:
            ...
            with self.conn_manager.transaction() as cursor:
                ...
                setClauses.append("updatedat = %s" if config.DB_TYPE == 'postgres' else "updatedat = ?")
                params.append(datetime.now())
                params.append(executionId)

                setClauseStr = ", ".join(setClauses)
                whereClause = "executionid = %s" if config.DB_TYPE == 'postgres' else "executionid = ?"
                
                query = f"UPDATE strategyexecution SET {setClauseStr} WHERE {whereClause}"
            ...
            with self.conn_manager.transaction() as cursor:
                ...
                for row in cursor.fetchall():
                    ...
                    for key, value in row_dict.items():
                        ...
                        if key in ['executionid', 'tokenid', 'tokenname', 'avgentryprice', 
                                   'remainingcoins', 'allotedamount', 'investedamount', 
                                   'amounttakenout', 'realizedpnl', 'realizedpnlpercent',
                                   'notes'] or key.startswith('execution_'):
                            execution_data[key] = value
                        else:
                            config_data[key] = value

                    # Add missing necessary fields if needed (adjust based on actual schema)
                    if 'strategyid' not in execution_data and 'strategyid' in config_data:
                    ...
                    execution_data['allotedamount'] = Decimal(str(execution_data['allotedamount']))
                    ...
                    config_data['strategyentryconditions'] = json.loads(config_data['strategyentryconditions'])
                    if config_data.get('chartconditions'):
                        config_data['chartconditions'] = json.loads(config_data['chartconditions'])
                    config_data['investmentinstructions'] = json.loads(config_data['investmentinstructions'])
                    config_data['profittakinginstructions'] = json.loads(config_data['profittakinginstructions'])
                    config_data['riskmanagementinstructions'] = json.loads(config_data['riskmanagementinstructions'])
                    if config_data.get('moonbaginstructions'):
                        config_data['moonbaginstructions'] = json.loads(config_data['moonbaginstructions'])
                    if config_data.get('additionalinstructions'):
                        config_data['additionalinstructions'] = json.loads(config_data['additionalinstructions'])
                        
                    config_data['status'] = StrategyStatus(config_data['status'])
                    config_data['active'] = bool(config_data['active'])
                    config_data['superuser'] = bool(config_data['superuser'])
    ...
    def getExecutionById(self, executionId: int) -> Optional[Dict]:
        ...
        try:
            ...
            with self.conn_manager.transaction() as cursor:
                params = (executionId,)
                if config.DB_TYPE == 'postgres':
                    query = text('SELECT * FROM strategyexecution WHERE executionid = %s')
                    cursor.execute(query, params)
                else:
